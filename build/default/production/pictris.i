...
# 1 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.00\\pic\\include\\c99\\limits.h" 1 3
# 10 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.00\\pic\\include\\c99\\limits.h" 3
# 1 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.00\\pic\\include\\c99\\bits/limits.h" 1 3
# 10 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.00\\pic\\include\\c99\\limits.h" 2 3
# 26 "pictris.c" 2
...
volatile
        _Bool Left_Debounced, Right_Debounced, Rotate_Debounced, Down_Debounced;
...
# 480 "pictris.c"
        if (mtxButtons)
        {
            ...
            debounceButton(PORTBbits.RB1, &Right_Delay, &Right_Debounced );
            debounceButton(PORTBbits.RB2, &Rotate_Delay, &Rotate_Debounced);
            debounceButton(PORTBbits.RB3, &Down_Delay, &Down_Debounced );
        }
...
void selectNextObject(volatile uint16_t *pTarget)
{
    ...
    s = &shapes[selection % 7];
    memcpy(pTarget, s->graphic, 16);
    OriginX = s->x;
    OriginY = s->y;
...
_Bool checkForLeftWall(volatile uint16_t * pObject)
{
    return pObject[0] != 0;
}
...
void splash_screen(void)
{
    ...
    if (tetris == 1){ memcpy(ObjectData, TETRIS, 16); }
...
void choosescreen(void)
{
    ...
    while (!Down_Debounced && !Rotate_Debounced)
        continue;
    uint16_t mask[8];
    if(Down_Debounced == 1) { tetris = 0;
    for (i=0;i<8;i++){ mask[i] = 0xFF00; }}
    else {if(Rotate_Debounced == 1) { tetris = 1;
...
{
    memcpy(pTarget, Number[pDigit], 16);
}
